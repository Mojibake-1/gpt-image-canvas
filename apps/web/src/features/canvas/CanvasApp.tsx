...
export function App() {
  ...
  const saveProjectSnapshot = useCallback(async (editor: Editor): Promise<void> => {
    ...
    try {
      const response = await apiFetch("/api/project", {
        ...
        body: JSON.stringify({
          snapshot: await persistDataUrlAssetsInSnapshot(
  ...
  function hasAgentJobPlaceholdersForRun(runId: string): boolean {
    for (const placeholderSet of agentJobPlaceholdersRef.current.values()) {
      if (placeholderSet.runId === runId) {
        return true;
      }
    }
  ...
  function isStaleAgentRunEvent(event: Pick<AgentServerEvent, "runId">): boolean {
    ...
    return !hasAgentJobPlaceholdersForRun(event.runId);
  }
  ...
  function finishAgentJobPlaceholdersFromOutputs(event: Extract<AgentServerEvent, { type: "job_completed" }>): void {
    ...
    event.outputs.forEach((output, index) => {
      ...
      const shapeId = replaceAgentPlaceholderAtIndex(editor, agentPlaceholderSet, outputIndex, output.asset, `${event.jobId}: ${output.asset.fileName}`);
      if (!shapeId) {
        addAgentOutputAssetToCanvas({
          type: "asset_preview",
