...
import { LOCAL_ONLY_PROVIDER_SOURCE_ORDER, isProviderSourceOrder } from "../../domain/providers/provider-config.js";
...
export function parseEditPayload(input: unknown, ownerEmail?: string): ParseResult<EditImageProviderInput> {
  const base = parseBaseImagePayload(input);
  if (!base.ok) {
    return base;
  }
...
export function parseProviderConfigPayload(
  ...
  const sourceOrder: ParseResult<ProviderSourceId[]> = options.localOnly
    ? { ok: true, value: [...LOCAL_ONLY_PROVIDER_SOURCE_ORDER] }
