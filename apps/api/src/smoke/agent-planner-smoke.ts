import {
  ...
  createCompatibleChatCompletionsPlanner,
...
async function main(): Promise<void> {
  ...
  await smokeCompatiblePlannerUsesSelectedSkills();
...
async function smokeCompatiblePlannerUsesSelectedSkills(): Promise<void> {
  ...
  try {
    const runner = createCompatibleChatCompletionsPlanner(
      llmConfigFixture({
        apiKey: "test-key",
  ...
  expect(isRecord(requestBody), "compatible planner sends a JSON request body");
  expect(Array.isArray(requestBody.messages), "compatible planner sends messages");
  const systemMessage = requestBody.messages[0];
  expect(isRecord(systemMessage), "compatible planner sends a system message");
  const systemPrompt = typeof systemMessage.content === "string" ? systemMessage.content : "";
  expect(
    systemPrompt.includes("ecommerce-visual-copywriting"),
    "compatible planner includes ecommerce skill for ecommerce system prompt"
  );
  expect(
    systemPrompt.includes(ECOMMERCE_VISUAL_COPYWRITING_COMPLIANCE_RULES_PATH),
    "compatible planner includes ecommerce compliance reference for ecommerce system prompt"
  );
}
