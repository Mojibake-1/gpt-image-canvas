...
const INTERRUPTED_GENERATION_ERROR = "生成被画布服务重启中断。你的图像 API 配置可能仍然正常，请从历史记录重新运行。";
const CANCELLED_GENERATION_ERROR = "This generation was cancelled.";
const DEFAULT_OWNER_EMAIL = "default";
...
export async function runTextToImageGeneration(
  ...
  ownerEmailOrSignal?: string | AbortSignal,
  signal?: AbortSignal,
  options: GenerationRunOptions = {}
): Promise<GenerationResponse> {
  const { ownerEmail, abortSignal } = resolveOwnerEmailAndSignal(ownerEmailOrSignal, signal);
  const outputs = await mapWithConcurrency(
    Array.from({ length: input.count }, (_, index) => index),
    input.count,
    async () => generateSingleOutput(input, provider, abortSignal, options)
  );
...
export async function runReferenceImageGeneration(
  ...
  const outputs = await mapWithConcurrency(
    ...
    async () => editSingleOutput(inputWithReferenceAssets, provider, abortSignal, options)
  );
...
export function createRunningTextToImageGeneration(input: ImageProviderInput, ownerEmail = DEFAULT_OWNER_EMAIL): GenerationRecord {
...
export async function finishTextToImageGeneration(
  ...
  const outputs = await mapWithConcurrency(
    Array.from({ length: input.count }, (_, index) => index),
    input.count,
    async () => generateSingleOutput(input, provider, abortSignal)
  );
...
export async function finishReferenceImageGeneration(
  ...
  const outputs = await mapWithConcurrency(
    Array.from({ length: input.count }, (_, index) => index),
    input.count,
    async () => editSingleOutput(input, provider, abortSignal, options)
  );
  throwIfAborted(abortSignal);
...
export function getGenerationRecord(generationId: string, ownerEmail?: string): GenerationRecord | undefined {
  return readGenerationRecord(generationId, ownerEmail);
}

export function cancelGenerationRecord(generationId: string, ownerEmail?: string): GenerationRecord | undefined {
  return updateGenerationRecordStatus(generationId, "cancelled", CANCELLED_GENERATION_ERROR, ownerEmail);
}
...
export async function saveImageDataUrlAsset(input: ReferenceImageInput, ownerEmail = DEFAULT_OWNER_EMAIL): Promise<GeneratedAsset> {
...
export async function saveReferenceImageInput(input: ReferenceImageInput, ownerEmail = DEFAULT_OWNER_EMAIL): Promise<GeneratedAsset> {
...
function createRunningGenerationRecord(input: PersistedGenerationInput, ownerEmail: string): GenerationRecord {
  const createdAt = new Date().toISOString();
  const generationId = input.clientRequestId || randomUUID();
  const existing = readGenerationRecord(generationId, ownerEmail);
  if (existing) {
    return existing;
  }
...
function completeGenerationRecord(
  ...
  insertGenerationOutputs(generationId, outputs, ownerEmail);
...
function insertGenerationOutputs(generationId: string, outputs: BatchOutputResult[], ownerEmail: string): void {
  const createdAt = new Date().toISOString();
...
function readGenerationRecord(generationId: string, ownerEmail?: string): GenerationRecord | undefined {
  const record = db
    .select()
    .from(generationRecords)
    .where(ownerEmail ? and(eq(generationRecords.id, generationId), eq(generationRecords.ownerEmail, ownerEmail)) : eq(generationRecords.id, generationId))
    .get();
  if (!record) {
    return undefined;
  }
...
function resolveOwnerEmailAndSignal(
  ownerEmailOrSignal: string | AbortSignal | undefined,
  signal: AbortSignal | undefined
): { ownerEmail: string; abortSignal?: AbortSignal } {
  if (typeof ownerEmailOrSignal === "string") {
    return {
      ownerEmail: ownerEmailOrSignal,
      abortSignal: signal
    };
  }
  ...
  return {
    ownerEmail: DEFAULT_OWNER_EMAIL,
    abortSignal: ownerEmailOrSignal ?? signal
  };
}
