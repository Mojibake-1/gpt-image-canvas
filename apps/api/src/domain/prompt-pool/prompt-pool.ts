...
export async function getPromptPool(promptPoolDirOverride?: string): Promise<PromptPoolResponse> {
  try {
    const { promptsPath, promptsStat, summaryPath } = await resolvePromptPoolFiles(promptPoolDirOverride);
...
async function resolvePromptPoolFiles(promptPoolDirOverride?: string): Promise<{ promptsPath: string; promptsStat: Awaited<ReturnType<typeof stat>>; summaryPath: string }> {
  const baseDir = promptPoolDirOverride ?? runtimePaths.promptPoolDir;
  const candidateDirs = [baseDir, resolve(baseDir, "data")];
