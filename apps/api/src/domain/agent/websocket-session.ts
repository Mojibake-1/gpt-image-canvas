...
export function createAgentWebSocketEvents(
  ...
  const { resumeFailedRunId, session } = resolveAgentSocketSession(connectionId, runId, ownerEmail, conversationId);
...
function createAgentSocketSession(ownerEmail: string, conversationId?: string): AgentSocketSession {
...
function resolveAgentSocketSession(
  ...
  if (connectionId) {
    ...
    if (existingSession?.ownerEmail === ownerEmail) {
  ...
  if (runId) {
    const activeRunSession = [...sessions.values()].find((session) => session.ownerEmail === ownerEmail && session.activeRun?.id === runId);
    if (activeRunSession) {
  ...
  const session = createAgentSocketSession(ownerEmail, conversationId);
  return {
    session,
...
async function handleAgentPlanMessage(
  ...
  try {
    clientSelectedReferences = await persistAgentSelectedReferences(rawClientSelectedReferences, session.ownerEmail);
  } catch (error) {
...
async function persistAgentSelectedReferences(
  ...
  return Promise.all(
    references.slice(0, MAX_AGENT_SELECTED_REFERENCES).map(async (reference) => {
      const storedAssetId = storedAssetIdForAgentReference(reference.assetId, ownerEmail);
      if (storedAssetId) {
        return {
          ...reference,
...
function storedAssetIdForAgentReference(assetId: string, ownerEmail: string): string | undefined {
  for (const candidate of storedAssetIdCandidates(assetId)) {
    const stored = getStoredAssetFile(candidate, ownerEmail);
    if (stored) {
      return stored.id;
    }
  }
